...
fk = conn.execute(
    "SELECT id, query_hash, collection, op_type FROM raw_query_slow_mongo WHERE raw_query_id=?",
    (rq_id,),
).fetchone()
print(f"  Result: {fk}")
...
if not is_json and qd:
    ...
    for h in [host, None, ""]:
        for d in [db_name, None, ""]:
            ...
            vrow = conn.execute(
                "SELECT id, host, db_name FROM raw_query_slow_mongo WHERE query_hash=?", (variant,)
            ).fetchone()
            if vrow:
                print(
                    f"  host={h!r} db={d!r} -> hash {variant[:12]}... -> typed id={vrow[0]} host={vrow[1]!r} db={vrow[2]!r}"
                )
...
if qd:
    ...
    if len(parts) >= 1:
        ...
        for c in candidates:
            print(
                f"    typed id={c[0]} hash={c[1][:12]} host={c[2]!r} db={c[3]!r} col={c[4]!r} op={c[5]!r}"
            )
