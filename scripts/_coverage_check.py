...
typed_texts = set(r[0] for r in conn.execute("SELECT command_json FROM raw_query_slow_mongo") if r[0])
print(f"Unique typed texts: {len(typed_texts):,}")

print("Checking raw_query coverage...")
rq_texts = set(r[0] for r in conn.execute("SELECT query_details FROM raw_query WHERE type='slow_query_mongo'") if r[0])
